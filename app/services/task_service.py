...
class TaskService:
    ...
    def clear_task_cached(self, user_id: UUID):
        ...
        cached = [f"tasks:{user_id}:*", f"tasks_scale:{user_id}:*"]
    ...
    def get_tasks(
        ...
        result = self.repository.get_tasks_raw(
            user_id, page, limit, status, priority, progress, search, sort_by, order
        )

        self.redis.set(
            cache_key, json.dumps(encoders.jsonable_encoder(result)), ex=300, nx=True
        )
    ...
    async def get_tasks_scale(
        ...
        if limit < 10 or limit > 100:
            raise HTTPException(
                status_code=400, detail="Limit must be between 10 and 100"
            )
        ...
        if is_locked:
            try:
                ...
                self.redis.set(
                    cache_key, json.dumps(encoders.jsonable_encoder(result)), ex=300
                )
    ...
    def delete_task(self, task_id, user_id: UUID):
        ...
        cached = self.redis.scan_iter(
            match=f"tasks:{user_id}:*"
        ) or self.redis.scan_iter(match=f"tasks_scale:{user_id}:*")
    ...
    def update_task(self, task_id, name, progress, user_id):
        ...
        task_updated = self.repository.update_raw(
            task, name, progress, updated_at, user_id
        )
